...
.header-container {
    background: linear-gradient(135deg, #0d3251 0%, #1a5276 100%);
    border-radius: 20px;
    padding: 20px;
    margin-bottom: 20px;
    color: white;
...
<div class="header-container">
    <div style="text-align: center;">
        <h1 style="color: white !important;">Práctica Virtual: Balance en estado no estacionario</h1>
        <p style="color: #d4e6f1 !important;">Escuela de Ingeniería Química | UCV</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar.container():
    ...
    with st.sidebar.expander("📐 Especificaciones del Tanque", expanded=True):
        ...
        # CORRECCIÓN PARA EL SLIDER: Evita error si h_total es pequeño
        min_limit = 0.05
        max_limit = float(h_total)
        sp_nivel = st.slider("Consigna de Nivel (Setpoint) [m]", 
                             min_value=min_limit, 
                             max_value=max_limit, 
                             value=float(h_total/2))
    ...
    if ajuste_manual:
        ...
        cd_manual = st.number_input("Cd Manual", value=cd_automatico, format="%.4f")
        st.session_state['cd_calculado'] = cd_manual

    with st.sidebar.expander("🛡️ Escenario de Perturbación", expanded=True):
        p_activa = st.toggle("Simular Falla/Fuga", value=False)
        if p_activa:
            p_tipo = st.selectbox("Tipo de Perturbación", ["Entrada", "Salida (Fuga)"])
            p_magnitud = st.number_input("Magnitud Qp [m³/s]", value=0.3, step=0.1)
            p_tiempo = st.slider("Inicio [s]", 0, 500, 100)
        else:
            p_magnitud, p_tiempo, p_tipo = 0.0, 0, "Entrada"

    with st.sidebar.expander("🎛️ Controlador PID", expanded=True):
        kp_sug, ki_sug, kd_sug = sintonizar_controlador_robusto(geom_tanque, r_max, h_total, st.session_state.cd_calculado, q_max, tipo_proceso)
        modo_auto = st.checkbox("🎯 Modo Robusto (Auto)", value=True)
        if modo_auto:
            kp_val, ki_val, kd_val = kp_sug, ki_sug, kd_sug
        else:
            kp_val = st.number_input("Kp", value=20.0)
            ki_val = st.number_input("Ki", value=4.0)
            kd_val = st.number_input("Kd", value=1.5)

    tiempo_ensayo = st.slider("Tiempo simulación [s]", 60, 600, 300)
...
if not st.session_state.ejecutando:
    ...
    for i, t_act in enumerate(vector_t):
        ...
        # Actualización de métricas en cada iteración
    ...
    # Tabla de resumen y descarga (manteniendo identación dentro del 'else' principal)
    st.markdown("---")
    ...
    st.download_button(
        ...
        file_name=f"simulacion_{tipo_proceso.lower()}.csv",
        mime="text/csv"
    )
    ...
    <p>Simulador de Control PID con Dos Válvulas | © 2026</p>
    </div>
    """, unsafe_allow_html=True)        
